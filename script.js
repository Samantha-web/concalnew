  ...
  return gk_fileData[filename] || "";
      ...
      function getContainerDetails() {
        ...
        function calculateLoadingDetails(
          ...
          details += `<tr><td style="background-color: #e8f5e9;"><font style="font-weight: bold;">Loading qty without flat: ${loadingQty1} Boxes</font></td></tr>`;
          details += `<tr><td>Number of columns: ${columns}<br/> Number of rows horizontally: ${rowsHorizontally}<br/> Number of rows vertically: ${rowsVertically}</td></tr>`;
          ...
          const flatRowsHorizontally = Math.floor(
            containerWidth / adjustedLength
          );
          const flatRowsVertically = Math.floor(
            containerHeight / adjustedHeight
          );
          const loadingQty2 =
            flatColumns * flatRowsHorizontally * flatRowsVertically;

          details += `<tr><td style="background-color: #fff8e1;"><b>Flat loading qty rest of the length: ${loadingQty2} Boxes</b></td></tr>`;
          details += `<tr><td>Number of flat columns rest length: ${flatColumns}<br/> Number of flat rows horizontally: ${flatRowsHorizontally}<br/> Number of flat rows vertically: ${flatRowsVertically}</td></tr>`;
          ...
          const flatRowsHeightLength = Math.floor(
            containerLength / adjustedLength
          );
          const loadingQty3 =
            flatColumnsHeight *
            flatRowsHeightHorizontally *
            flatRowsHeightLength;

          details += `<tr><td style="background-color: #e3f2fd;"><b>Flat loading qty rest of the height: ${loadingQty3} Boxes</b></td></tr>`;
          details += `<tr><td>Number of flat columns rest height: ${flatRowsHeightLength}<br/> Number of flat rows horizontally: ${flatRowsHeightHorizontally}<br/> Number of flat rows vertically: ${flatColumnsHeight}</td></tr>`;
          details += `<tr><td style="background-color: #ffebee;"><font style="font-weight: bold; color: #d32f2f;">Total Loading Qty with flat: ${
          ...
          // Update table cells
          document.getElementById(`utilizedCbmVal${index}`).innerText =
            utilizedCbm;
        ...
        const cartons = Array.from({ length: 12 }, (_, i) => i + 1).map(
          (i) => ({
            cartonLength:
              parseFloat(document.getElementById(`cartonLength${i}`).value) ||
              0,
            cartonWidth:
              parseFloat(document.getElementById(`cartonWidth${i}`).value) || 0,
            cartonHeight:
              parseFloat(document.getElementById(`cartonHeight${i}`).value) ||
              0,
            orderQty:
              parseFloat(document.getElementById(`orderQty${i}`).value) || 0,
            bulgingLength:
              parseFloat(document.getElementById(`bulgingLength${i}`).value) ||
              0,
            bulgingWidth:
              parseFloat(document.getElementById(`bulgingWidth${i}`).value) ||
              0,
            bulgingHeight:
              parseFloat(document.getElementById(`bulgingHeight${i}`).value) ||
              0,
            index: i,
          })
        );
        ...
        totalCartonQty = cartons.reduce(
          (sum, carton) => sum + carton.orderQty,
          0
        );
        ...
        for (let i = 0; i < cartons.length; i += 2) {
          resultsHTML += `<table border='1' style='width:100%'><tr><th><br/>Carton Size ${String(
            i + 1
          ).padStart(2, "0")} Details</th><th><br/>Carton Size ${String(
            i + 2
          ).padStart(2, "0")} Details</th></tr>`;
          resultsHTML += "<tr>";
          resultsHTML += `<td>${calculateLoadingDetails(
